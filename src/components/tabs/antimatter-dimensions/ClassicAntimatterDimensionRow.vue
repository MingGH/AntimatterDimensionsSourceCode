...
export default {
  ...
  computed: {
    ...
    until10Text() {
      if (this.isCapped) return "被无名者摧毁";
      if (this.isContinuumActive) return `连续体: ${this.continuumString}`;

      const prefix = `直到 ${formatInt(10)},${this.showCostTitle(this.until10Cost) ? " 花费" : ""}`;
      const suffix = this.isCostsAD ? `${this.costUnit}` : "反物质";
    ...
    boughtTooltip() {
      if (this.isCapped) return `无名者阻止购买超过 ${format(1)} 个第八反物质维度`;
      if (this.isContinuumActive) return "连续体正在生产你所有的反物质维度";
      return `已购买 ${quantifyInt("次", this.bought)}`;
    },
    costUnit() {
      return `${AntimatterDimension(this.tier - 2).shortDisplayName} 反物质维度`;
    },
    buySingleClass() {
      return {
